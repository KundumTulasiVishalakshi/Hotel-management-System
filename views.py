...
def add_employee(request):
    ...
    role = str(group) if group else "NoRole"
    path = role + "/"
...
def guest_profile(request, pk):
    tempUser = get_object_or_404(User,id=pk)
    try:
        guest = Guest.objects.filter(user=tempUser)
    except Guest.DoesNotExist:
        messages.error(request, " guest profile does not exist.")
